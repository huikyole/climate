...
class SpatialPatternTaylorDiagram(BinaryMetric):
    ...
    def run(self, ref_dataset, target_dataset):
        ...
        '''
        if ref_dataset.values.ndim <= 2 and target_dataset.values.ndim <= 2:
            return ma.array([calc_stddev_ratio(target_dataset.values, ref_dataset.values), calc_correlation(ref_dataset.values, target_dataset.values)])
        else:
