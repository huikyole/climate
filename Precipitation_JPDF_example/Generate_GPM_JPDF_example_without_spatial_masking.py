...
""" Step 1: Load the GPM datasets """

GPM_dataset = local.load_GPM_IMERG_files(file_path='./GPM_2015_summer/', filename_pattern=['*2015*.HDF5'])
...
""" Step 3: Spatial subset the GPM dataer (for Northern Great Plains, user_mask_values=[10]) """

GPM_dataset_filtered = dsp.subset(GPM_dataset, Bukovsky_mask, user_mask_values=[10])

""" Step 4: Analyze the wet spells """

spell_duration, peak_rainfall, total_rainfall = metrics.wet_spell_analysis(GPM_dataset_filtered.values, threshold=0.1, nyear=1, dt=0.5)
